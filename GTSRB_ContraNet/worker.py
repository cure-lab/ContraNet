"""
in this version I fixed the d2discriminator to improve the generator.
"""
...
#adding encoder to guide generator
...
LOG_FORMAT = (
	...
	# "ada_p: {ada_p:<.3} "
	"Dis_loss: {dis_loss:<.4}  "
	#"d2Dis_loss: {d2dis_loss:<.4}  "
	"Gen_loss: {gen_loss:<.4} "
	"Gen_loss_valina: {gen_acml_loss_watch_g_loss:<.4}  "
	#"Gen_loss_d2d: {gen_acml_loss_watch_g_loss_2:<.4}  "
	"L2: {gen_l2_loss_watch:<.4}  "
	"L1: {gen_l1_loss:<.4}  "
	"SSIM: {gen_ssim_loss_watch:<.4}  "
	"latent: {gen_lat_loss:<.4}  "
	"ori_overall: {gen_ori_loss:<.4}  "
...
class make_worker(object):
	...
	def train(self, current_step, total_step):
		...
		while step_count <= total_step:
			...
			for step_index in range(self.g_steps_per_iter):
				...
				for acml_step in range(self.accumulation_steps): #accumulation_steps=1
					...
					with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
						...
						gen_acml_loss = self.G_loss(dis_out_fake) + loss_img_l2 + loss_img_ssim + loss_lat# + loss_img_l1 
						if self.latent_op:
			...
			if step_count % self.save_every == 0 or step_count == total_step:
				if self.evaluate:
					is_best = False#self.evaluation(step_count, False, "N/A")
	...
	################################################################################################################################
	def evaluation(self, step, standing_statistics, standing_step): # standing_statistics=False standing_step=NA
		...
		with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
			...
			fid_score, self.m1, self.s1 = 0,0,1#calculate_fid_score(self.eval_dataloader, generator, self.dis_model, self.inception_model, self.num_eval[self.eval_type],
			...
			kl_score, kl_std = 1,1
			# calculate_incep_score(self.eval_dataloader, generator, self.dis_model, self.inception_model, self.num_eval[self.eval_type],
			# 										 self.truncated_factor, self.prior, self.latent_op, self.latent_op_step4eval, self.latent_op_alpha,
			# 										 self.latent_op_beta, num_split, self.local_rank, self.logger)

			precision, recall, f_beta, f_beta_inv = 0,0,0,0
			# calculate_f_beta_score(self.eval_dataloader, generator, self.dis_model, self.inception_model, self.num_eval[self.eval_type],
