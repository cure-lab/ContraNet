...
# from resnet import ResNet10APP
...
def main():
	...
	parser.add_argument('-c', '--config_path', type=str, default='./configs/CIFAR10/GTSRB_config.json')
	parser.add_argument('-n', '--nodes', default=1, type=int, metavar='N')
	parser.add_argument('--checkpoint_folder', default="./checkpoints/bestversionGTSRB2/GTSRB_config-train-2021_04_15_10_35_52")
	parser.add_argument('--load_current', default=True)

	parser.add_argument('--log_output_path', default="./adv_train_discriminator_logs")
	...
	local_rank = rank
	# adding classifier
	checkpoint_path = '/research/dept6/yjyang/SP2020/GTSRB/GTSRBclassifier'
	classifier = ResNet18()# ResNet10APP(np.around(float(1.0),3), 43).to(local_rank)
	classifier_path = os.path.join(checkpoint_path,"ResNet-18.t7")
	classifier_ckpt = torch.load(classifier_path)
	classifier.load_state_dict(classifier_ckpt["net"])
	
	print(classifier_ckpt["acc"])
	...
	writer = SummaryWriter(log_dir=join("./GTSRBmydiscriminator_logs", run_name)) if local_rank == 0 else None
	if local_rank == 0:
	...
	if cfgs.checkpoint_folder is None:
		...
		writer = SummaryWriter(log_dir=join('./GTSRB_mydiscriminator_logs', run_name)) if global_rank == 0 else None
