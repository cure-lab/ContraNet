...
class make_worker(object):
    def __init__(self, cfgs, run_name, best_step, logger, writer, n_gpus, gen_model, dis_model, inception_model, Gen_copy,
                 ...
                 best_fid_checkpoint_path, encoder, opt_encoder):
    ...
    def train(self, current_step, total_step):
        ...
        while step_count <= total_step:
            # ================== TRAIN D ================== #
            toggle_grad(self.dis_model, on=False, freeze_layers=-1)
            ...
            for step_index in range(self.d_steps_per_iter):
                ...
                for acml_index in range(self.accumulation_steps):
                    ...
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        zs_invencoder = self.encoder(fake_images, fake_labels)

                        encoder_loss = self.l2_loss(zs_invencoder, zs)
            ...
            if step_count % self.print_every == 0 and self.global_rank == 0:
                    ...
                    inv_encoder_images = self.gen_model(zs_invencoder, fake_labels)
                    
                    self.encoder.eval()
                    zs_real = self.encoder(real_images, real_labels)
                    real_reconstruction_images = self.gen_model(zs_real, real_labels)
                    self.encoder.train()
                    ...
                    self.writer.add_image(
                        'invencoder', torchvision.utils.make_grid(
                            inv_encoder_images, nrow=8, normalize=True, scale_each=True), global_step=step_count)
                    self.writer.add_image(
                        'real', torchvision.utils.make_grid(
                            real_images, nrow=8, normalize=True, scale_each=True), global_step=step_count)
                    self.writer.add_image(
                        'real_reconstruction', torchvision.utils.make_grid(
                            real_reconstruction_images, nrow=8, normalize=True, scale_each=True), global_step=step_count)
            ...
            if step_count % self.print_every == 0 and self.global_rank == 0:
                ...
                self.writer.add_scalars('Losses', {'encoder': encoder_loss.item()}, step_count)
                if self.ada:
    ...
    def save(self, step, is_best):
        ...
        if isinstance(self.encoder, DataParallel) or isinstance(self.encoder, DistributedDataParallel):
            enc = self.encoder.module
            # if self.Gen_copy is not None:
            #     gen_copy = self.Gen_copy.module
        else:
            enc = self.encoder#, self.dis_model
            # if self.Gen_copy is not None:
        ...
        e_states = {'seed': self.seed, 'run_name': self.run_name, 'step': step, 'best_step': self.best_step,
                    'state_dict': enc.state_dict(), 'optimizer': self.opt_encoder.state_dict(), 'ada_p': self.ada_aug_p,
                    'best_fid': self.best_fid, 'best_fid_checkpoint_path': self.checkpoint_dir}

        if len(glob.glob(join(self.checkpoint_dir,"model=E-{when}-weights-step*.pth".format(when=when)))) >= 1:
        ...
        if when == "best":
            if len(glob.glob(join(self.checkpoint_dir,"model=E-current-weights-step*.pth"))) >= 1:
