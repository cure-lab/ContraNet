...
if __name__ == "__main__":
    ...
    parser.add_argument('--arch', default="ContraNet2_3l2H")
    parser.add_argument(
    ...
    parser.add_argument('--log_dir', type=str, default='results/log/roc/AA')
    ...
    save_name = os.path.join(args.log_dir, "{}_roc.pdf".format(
        args.arch + config_name + "_" + args.model))
    ...
    if os.path.exists(load_path):
        ...
        with torch.no_grad():
            # Note that model from model zoo do not need normalize.
            score_dict_clean = collect_clean(
                test_loader, model.classifier, model)
    ...
    if args.csv_file is not None:
        final_results = args.arch + "\t" + args.model + "\t" + "\t".join(
